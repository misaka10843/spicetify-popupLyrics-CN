...
function PopupLyrics() {
  ...
  const userConfigs = {
      ...
      services: {
          netease: {
              ...
              desc: `由中国开发者和用户运营的众包歌词提供商。`,
          },
          neteaseCN: {
            on: boolLocalStorage("popup-lyrics:services:neteaseCN:on"),
            call: LyricProviders.fetchNeteaseCN,
            desc: `在有中文翻译时使用网易云音乐翻译，没有时使用spotify官方歌词`,
        },
          
          musixmatch: {
              on: boolLocalStorage("popup-lyrics:services:musixmatch:on"),
              call: LyricProviders.fetchMusixmatch,
              desc: `与Spotify完全兼容。需要可以从官方Musixmatch应用程序中检索的令牌。遵循 <a href="https://github.com/khanhas/spicetify-cli/wiki/Musixmatch-Token">spicetify Wiki</a>.上的说明`,
          ...
          spotify: {
              ...
              desc: `Spotify官方提供的歌词。仅适用于某些地区/国家的用户（例如，日本、越南、泰国）。`,
          },
      },
      servicesOrder: [],
  };
  ...
  const button = new Spicetify.Topbar.Button("悬浮歌词", "lyrics", () => {
      if (!lyricVideoIsOpen) {
